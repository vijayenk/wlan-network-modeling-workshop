...
% Define properties in order of intended display
    properties
        ...
        %   packet. When enabled phase error gradient (PEG) is tracked in a
        %   packet.
        PilotTimeTracking (1,1) logical = true;
        %PilotPhaseTracking Data field pilot phase (carrier frequency
        %offset) tracking
        %   Enable or disable pilot phase (carrier frequency offset)
        %   tracking during the data portion of a packet. The default is
        %   true. Pilot time tracking is not performed during other
        %   portions of a packet. When enabled, common phase error (CPE) is
        %   tracked in a packet.
        ...
        %DataAidedEqualization Data-aided equalization using
        %demodulated LTF and data symbols
        %   Enable or disable data-aided equalization. The default is
        %   false. When enabled, the channel estimates are refined using
        %   the demodulated LTF and data symbols and used for equalization.
        DataAidedEqualization (1,1) logical = false;
        %IQImbalanceCorrection IQ imbalance estimation and correction
        %   Enable or disable IQ imbalance estimation and correction. The
        %   default is false. When enabled, the IQ imbalance is estimated
        %   and corrected in the data symbols.
    ...
    methods
        ...
        % Use trackingRecoveryConfig validator
            wirelessWaveformAnalyzer.internal.trackingRecoveryConfig('EqualizationMethod',val);
            obj.EqualizationMethod = val;
        end

        function set.PilotTrackingWindow(obj,val)
        % Use trackingRecoveryConfig validator
            wirelessWaveformAnalyzer.internal.trackingRecoveryConfig('PilotTrackingWindow',val);
        ...
        %   It must be 'CBW20', 'CBW40', 'CBW80', 'CBW160', or 'CBW320'.
        %
        ...
        % Known channel bandwidth
            ...
            if resampleWaveform
                ...
                rateConv = dsp.FarrowRateConverter('InputSampleRate',obj.wavsr,'OutputSampleRate',bbsr);
                rxWaveform = rateConv(waveform);
            ...
            % The TXTIME for a VHT/HT packet with short GI is given by
            % Equation 21-109 of IEEE Std 802.11-2020. Due to the ceiling
            % operation in Equation 21-109, the number of samples in the
            % packet calculated using TXTIME maybe greater than the actual
            % number of samples in the waveform. The TXTIME is the same as
            % the RXTIME. On reception of a short GI packet, the number of
            % samples in the packet maybe less than the number of samples
            % calculated using RXTIME.

            % The received waveform is padded with dummy samples equal to
            % the number of OFDM samples required by a single data symbol
            % for a short GI. This is to ensure that the number of samples
            % in the receive packet is greater than the number of samples
            % calculated with RXTIME.

            nfft = wlan.internal.cbw2nfft(cfgRef.ChannelBandwidth);
            rxWaveform = [rxWaveform; zeros(nfft*9/8,width(rxWaveform))];
            [rxWavLen,numRx] = size(rxWaveform);

            % Data recovery options
            dataRecOpt = wirelessWaveformAnalyzer.internal.trackingRecoveryConfig;
            dataRecOpt.EqualizationMethod = obj.EqualizationMethod;
            dataRecOpt.PilotTrackingWindow = obj.PilotTrackingWindow;
            dataRecOpt.PilotTimeTracking = obj.PilotTimeTracking;
            dataRecOpt.PilotPhaseTracking = obj.PilotPhaseTracking;
            dataRecOpt.PilotGainTracking = obj.PilotGainTracking;
            dataRecOpt.DataAidedEqualization = obj.DataAidedEqualization;
            dataRecOpt.IQImbalanceCorrection = obj.IQImbalanceCorrection;
            ...
            while state.nextState ~= "NoPacketDetected"
                switch state.nextState
                  case "Preamble"
                    % Detect packets
                    [state.status,preRes] = wirelessWaveformAnalyzer.internal.recoverPreamble(rxWaveform,chanBW,searchOffset,preCfg);
                  ...
                  case "L-SIG"
                    ...
                    fmtDetInd = preRes.PacketOffset+(1:commonIndices.USIG(2));
                    ...
                    fmtDetectBuffer = frequencyOffset(fmtDetectBuffer,bbsr,-preRes.CFOEstimate);
                    ...
                    [status,lsigRes] = wirelessWaveformAnalyzer.internal.recoverLSIG(fmtDetectBuffer(commonIndices.LSIG(1):end,:),preRes.ChanEstNonHT,preRes.NoiseEstNonHT,chanBW,format);
                    ...
                    elseif preRes.PacketOffset+packetRes.LSIG.NumRxSamples>rxWavLen
                        ...
                        rxPacket = frequencyOffset(rxPacket,bbsr,-preRes.CFOEstimate);
                        ...
                        if ~strcmpi(format,"EHT-MU")
                            % Force disable IQ imbalance correction for
                            % formats other than "EHT-MU"
                            dataRecOpt.IQImbalanceCorrection = false;
                        end
                  ...
                  case "EHT"
                    [state,rxPSDU,cfgRx,res] = wirelessWaveformAnalyzer.internal.recoverEHT(rxPacket,packetRes.LSIG.PreEHTChanEst,preRes.NoiseEstNonHT,chanBW,packetRes.LSIG.Info.Length,dataRecOpt,lsigRes.IQImbalance);

                    % Adjust measured powers to account for L-STF AGC scaling
                    res = adjustEHTPowersForLSTFAGC(res,preRes.LSTFPower);

                    packetRes.PHYConfig = cfgRx;
                    packetRes.USIG = res.USIG;
                    packetRes.EHTSIG = res.EHTSIG;
                    packetRes.EHTPreamble = res.EHTPreamble;
                    packetRes.EHTData = res.EHTData;
                    packetRes.PSDU = rxPSDU;

                  case "HE"
                    [state,rxPSDU,cfgRx,res] = wirelessWaveformAnalyzer.internal.recoverHE(rxPacket,packetRes.LSIG.PreHEChanEst,preRes.NoiseEstNonHT,chanBW,packetRes.LSIG.Info.Length,format,dataRecOpt);
                  ...
                  case "Non-HT"
                    [rxPSDU,cfgRx,resNonHTData] = wirelessWaveformAnalyzer.internal.recoverNonHT(rxPacket,preRes.ChanEstNonHT,preRes.NoiseEstNonHT,chanBW,packetRes.LSIG.Info.MCS,packetRes.LSIG.Info.Length,dataRecOpt,preRes.DemodLLTF);
                  ...
                  case "VHT"
                    [state,rxPSDU,cfgRx,res] = wirelessWaveformAnalyzer.internal.recoverVHT(rxPacket,preRes.ChanEstNonHT,preRes.NoiseEstNonHT,chanBW,packetRes.LSIG.Bits,dataRecOpt);
                  ...
                  case "HT-MF"
                    [state,rxPSDU,cfgRx,res] = wirelessWaveformAnalyzer.internal.recoverHTMF(rxPacket,preRes.ChanEstNonHT,preRes.NoiseEstNonHT,chanBW,dataRecOpt);
                  ...
                  case "A-MPDU"
                    ...
                    isEHTPacket = strcmp(format,'EHT-MU');
                    for u = 1:numUsers
                        % Deaggregate the A-MPDU
                        if isEHTPacket
                            % wlanAMPDUDeaggregate does not support wlanEHTRecoveryConfig now
                            [mpduList,~,deagstatus] = wlanAMPDUDeaggregate(rxPSDU{u},wlanHESUConfig);
                        else
                        ...
                        for i = 1:numel(mpduList)
                            if isEHTPacket
                                % wlanMPDUDecode does not support wlanEHTRecoveryConfig now
                                [mpduConfig,mpduPayload,mpduDecodeStatus] = wlanMPDUDecode(mpduList{i},wlanHESUConfig,'DataFormat','octets');
                            else
...
% on the format
    ...
    if strcmp(format,"EHT-MU")
        packetRes.IQImbalance = lsigRes.IQImbalance;
    end
    if any(strcmp(format,["HE-SU","HE-MU","HE-EXT-SU","HE-TB","EHT-MU"]))
    ...
    switch format
      case "EHT-MU"
        packetRes.USIG = struct('Processed',false);
        packetRes.EHTSIG = struct('Processed',false,'User',struct('Processed',false));
        packetRes.EHTPreamble = struct('Processed',false);
        packetRes.EHTData = struct('Processed',false);
      case {"HE-SU","HE-MU","HE-EXT-SU"}
...
function res = adjustEHTPowersForLSTFAGC(res,lstfPower)
% Power measurements within EHT processing function are not aware that
% packet has been scaled by L-STF AGC, so adjust power measurements
% accordingly
    res.USIG.Power = res.USIG.Power*lstfPower;
    res.EHTSIG.Power = res.EHTSIG.Power*lstfPower;
    res.EHTPreamble.EHTSTFPower = res.EHTPreamble.EHTSTFPower*lstfPower;
    res.EHTPreamble.EHTLTFPower = res.EHTPreamble.EHTLTFPower*lstfPower;
    res.EHTData.Power = res.EHTData.Power*lstfPower;
    for r = 1:numel(res.EHTPreamble.RU)
        res.EHTPreamble.RU(r).Power = res.EHTPreamble.RU(r).Power*lstfPower;
    end
    for u = 1:numel(res.EHTData.User)
        res.EHTData.User(u).Power = res.EHTData.User(u).Power*lstfPower;
    end
    for r = 1:numel(res.EHTData.RU)
        res.EHTData.RU(r).Power = res.EHTData.RU(r).Power*lstfPower;
    end
end
