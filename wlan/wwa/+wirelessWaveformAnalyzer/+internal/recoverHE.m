        ...
        switch state.nextState
          ...
          case "HE-Data"
            ...
            [psdu, res.HEData] = processHEData(rxPacket, ...
                                               res.PHYConfig,res.HEPreamble,cfgDataRec, ...
                                               indexHE);
...
% Process HE-SIG-A and HE-SIG-B
    ...
    preheInfo = wlan.internal.heOFDMInfo('HE-SIG-A',chanBW);
    sigaDemod = wlan.internal.ofdmDemodulate(rxSIGA,preheInfo,0.75);
    ...
    if strcmp(cfgRx.PacketFormat,'HE-MU')
        ...
        if ~cfgRx.SIGBCompression
            ...
            preheInfo = wlan.internal.heOFDMInfo('HE-SIG-B',chanBW);
            demodCommonSym = wlan.internal.ofdmDemodulate(rxCommon,preheInfo,0.75);
            ...
            % Find punctured and active 20 MHz subchannels
            puncSubchan = [];
            if (strcmp(cfgRx.ChannelBandwidth,'CBW80') || strcmp(cfgRx.ChannelBandwidth,'CBW160')) && ~failInterp
                % 20 MHz puncturing
                % Punctured subchannels with allocation index 113
                puncSubchan = [puncSubchan find(cfgRx.AllocationIndex==113)];
            end
            if strcmp(cfgRx.ChannelBandwidth,'CBW160') && ~failInterp
                % 40 MHz puncturing
                % Two adjacent 20 MHz subchannels with allocation indices
                % of [114 114]
                pairAllocIdx = reshape(cfgRx.AllocationIndex,2,[]);
                index40MHzPunc = find(sum(pairAllocIdx,1)==228 & any(pairAllocIdx==114));
                if ~isempty(index40MHzPunc)
                    % Punctured subchannels
                    puncSubchan = [puncSubchan (index40MHzPunc-1)*2+(1:2)];
                end
            end
            % Initialize by assuming all subchannels are active
            activeSubchan = true(preheInfo.NumSubchannels,1);
            % Disable the flag for punctured subchannels
            activeSubchan(puncSubchan) = false;
            % Repeat the active subchannel values to obtain active data
            % subcarriers
            activeDataSubC = repelem(activeSubchan,52);

            [EVMRMS,EVMMax] = EVM(res.HESIGB.Common.EQDataSym(activeDataSubC,:));
            ...
            % All subchannels are active for non-OFDMA waveforms (HE-SIG-B
            % Compression is ON)
            activeSubchan = true(preheInfo.NumSubchannels,1);
            activeDataSubC = repelem(activeSubchan,52);
        end

        % Update EVM for HE-SIG-A field considering only active
        % subcarriers (non-punctured)
        [EVMRMS,EVMMax] = BPSKEVM(eqSIGASym(activeDataSubC,:));
        res.HESIGA.EVMRMS = 20*log10(mean(EVMRMS)/100);
        res.HESIGA.EVMMax = 20*log10(mean(EVMMax)/100);
        ...
        preheInfo = wlan.internal.heOFDMInfo('HE-SIG-B',chanBW);
        demodUserFieldData = wlan.internal.ofdmDemodulate(rxHESIGB,preheInfo,0.75);
        ...
        [EVMRMS,EVMMax] = EVM(res.HESIGB.EQDataSym(activeDataSubC,:));
...
% HE-STF AGC - apply to remaining packet
    ...
    heltfRU = struct('RUSize',0,'RUIndex',0,'UserNumbers',[],'UserIndices',[],'ChanEst',[],'PilotEst',[],'Power',0,'HELTFDemod',[]);
    ...
    for r = 1:allocInfo.NumRUs
        ...
        % HE-LTF demodulation
        heltfInfo = wlan.internal.heOFDMInfo('HE-LTF',cfgUser(1).ChannelBandwidth,cfgUser(1).GuardInterval,allocInfo.RUSizes(r),allocInfo.RUIndices(r));
        heltfDemod = wlan.internal.ehtLTFDemodulate(rxHELTF,cfgUser(1).HELTFType,0.75,heltfInfo);
        ...
        heltfRU(r).HELTFDemod = heltfDemod;
    end
...
function [rxPSDU, resData] = processHEData(rxPacket,users,hePreamble,cfgDataRec,indexHE)
...
% Extract data portion from packet
    ...
    heOFDMInfo = wlan.internal.heOFDMInfo('HE-Data',users(1).ChannelBandwidth,users(1).GuardInterval);
    ...
    for u = 1:numUsers
        ...
        [rxPSDU{u},resUser(u)] = processHEDataUser(RU(ruNum).HELTFDemod, rxHEData, ...
                                                   RU(ruNum).ChanEst,RU(ruNum).PilotEst,users(u),numOFDMSym,cfgDataRec,u);
...
function [rxPSDU,res] = processHEDataUser(heltfDemod,rxData,chanEst,pilotEst,user,numOFDMSym,cfgDataRec,userIdx)
% Process HE-Data field for a user
    ...
    res = struct;
    heOFDMInfo = wlan.internal.heOFDMInfo('HE-Data',user.ChannelBandwidth,user.GuardInterval,user.RUSize,user.RUIndex);

    % Joint demodulation and tracking
    if strcmp(user.PacketFormat,'HE-MU')
        s = getSIGBLength(user);
        numSIGSyms1 = s.NumSIGBSymbols; % Number of HE-SIG-B symbols
    else % SU or EXT_SU
        numSIGSyms1 = 0; % Number of HE-SIG-B symbols
    end
    if strcmp(user.PacketFormat,'HE-EXT-SU')
    ...
    fnRefPilots = @()wlan.internal.hePilots(user.RUSize,nsts,n,z);
    fnOFDMDedmodulate = @(x)wlan.internal.ofdmDemodulate(x,heOFDMInfo,cfgDataRec.OFDMSymbolOffset);

    % Average over number of symbols for SSPilot chanEst
    chanEstPilotsUse = mean(pilotEst,2);
    [demodSym,res.CPE,res.PEG,res.PilotGain] = wirelessWaveformAnalyzer.internal.trackingOFDMDemodulate(rxData,chanEstPilotsUse,fnRefPilots,fnOFDMDedmodulate,numOFDMSym,heOFDMInfo,cfgDataRec);
    ...
    % Data-aided channel estimation using demodulated HE-LTF and HE-Data symbols
    if cfgDataRec.DataAidedEqualization
        chanEst(heOFDMInfo.DataIndices,:,:) = wlan.internal.heDataAidedChannelEstimate(heltfDemod(heOFDMInfo.DataIndices,:,:),demodSym(heOFDMInfo.DataIndices,:,:),chanEst(heOFDMInfo.DataIndices,:,:),nEstEQ,user,userIdx);
    end
