function [state,psdu,cfgRx,res] = recoverHTMF(rxPacket,chanEstNonHT,nVarEstNonHT,chanBW,cfgDataRec)
...
while all(state.nextState~=["RxError" "A-MPDU" "MPDU" "RxSuccess"])
    switch state.nextState
        ...
        case "HT-SIG Evaluate"
            ...
            [cfgRx,failInterp] = wirelessWaveformAnalyzer.internal.htConfigRecover(resHTSIG.Bits,true);
        ...
        case "HT-Preamble"
            ...
            % Always extract NumSpaceTimeStreams as chanEst contains numEss for spatial expansion case
            chanEst = chanEst(:,1:cfgRx.NumSpaceTimeStreams,:);
        ...
        case "HT-Data"
            ...
            [psdu,eqDataSym,cpe,peg,pilotGain,nVarEst,eqPilotSym] = wirelessWaveformAnalyzer.internal.trackingHTDataRecover(rxData,chanEst,cfgRx,cfgDataRec,demodHTLTF);
...
[~,vhtData,vhtPilots] = wlan.internal.getOFDMConfig(cfgRx.ChannelBandwidth,'Long','HT');
...
[eqCombDataSym, csiData] = wlan.internal.equalize(ofdmOutDataOne20MHz, chanEstDataOne20MHz, recParams.EqualizationMethod, noiseVarEst);
...
[bits, failCRC] = wlan.internal.htSIGBitRecover(eqCombDataSym, noiseVarEst, csiData);
end