...
while all(state.nextState~=["RxError" "A-MPDU" "RxSuccess"])
    switch state.nextState
        ...
        case "VHT-SIG-A Evaluate"
            ...
            [cfgRx,numDataSym,~,~,failInterp] = wlan.internal.vhtConfigRecover(recLSIGBits,resVHTSIGA.Bits,'SuppressError',true);
        ...
        case "VHT-SIG-B"
            ...
            for u = 1:numUsers
                ...
                resVHTSIGBUser(u).ReferenceCRC = wlan.internal.vhtInterpretSIGB(recVHTSIGBBits,chanBW,numUsers==1);
            end
            ...
            for u = 1:numUsers
                [cfgRx(u),~,~,~,failInterp(u)] = wlan.internal.vhtConfigRecover(recLSIGBits,resVHTSIGA.Bits,resVHTSIGBUser(u).Bits,u,'SuppressError',true);
        ...
        case "VHT-Data"
            ...
            vhtOFDMInfo = wlan.internal.vhtOFDMInfo('VHT-Data', chanBW, cfgRx(1).GuardInterval);
            pilotInd = vhtOFDMInfo.PilotIndices;
            ...
            for u = 1:numUsers
                ...
                [psdu{u},rxSIGBCRC,eqDataSym,cpe,peg,pilotGain,nVarEst,eqPilotSym] = wirelessWaveformAnalyzer.internal.trackingVHTDataRecover(rxData, ...
                    chanEst,chanEstSSPilots,cfgRx(u),cfgDataRec,u,numSTSVec,numDataSym,demodVHTLTF);
...
[~,vhtData,vhtPilots] = wlan.internal.getOFDMConfig(cfgRx.ChannelBandwidth,'Long','VHT');
Nst = numel(vhtData)+numel(vhtPilots);
[~,nonhtData,nonhtPilots] = wlan.internal.getOFDMConfig(cfgRx.ChannelBandwidth,'Long','Legacy');
...
[eqDataCombSym, csiCombData] = wlan.internal.equalize(ofdmOutDataOne20MHz, chanEstDataOne20MHz, recParams.EqualizationMethod, noiseVarEst); % [48, 2]
eqDataSym = wlan.internal.equalize(ofdmOutData, chanEstData, recParams.EqualizationMethod, noiseVarEst); % Without combining

[bits, failCRC] = wlan.internal.vhtSIGABitRecover(eqDataCombSym, noiseVarEst, csiCombData);
end