...
%   'CBW160', or 'CBW320'.
%
%   FORMAT is the format of the packet and must be 'EHT-MU', 'HE-SU',
%   'HE-MU', 'HE-EXT-SU', 'HE-TB', 'VHT', 'HT-MF', or 'Non-HT'.
...
if isHE || isEHT
    ...
    [lsigBits,failCheck,preEHTChanEst,eqCombDataSym,lsigInfo,eqSepDataSym,iqImbalance,activeDataSubCarriers] = ehtProcessLSIG(rxLSIG(1:2*lenLSIG,:),chanEst,noiseVarEst,chanBW,format);
    ...
    %#exclude step
    [EVMRMS,EVMMax] = step(BPSKEVM,eqSepDataSym(activeDataSubCarriers,2)); % RL-SIG
    res.RLSIG.EVMRMS = 20*log10(mean(EVMRMS)/100);
    res.RLSIG.EVMMax = 20*log10(mean(EVMMax)/100);
    res.RLSIG.EQDataSym = eqSepDataSym(:,2);
else
    % Process LSIG
    [lsigBits,failCheck,eqCombDataSym,eqDataSym,activeDataSubCarriers] = lsigRecover(rxLSIG(1:lenLSIG,:),chanEst,noiseVarEst,chanBW,format);
...
[EVMRMS,EVMMax] = step(BPSKEVM,eqDataSym(activeDataSubCarriers,:));
...
cfgNonHT = wlanNonHTConfig("MCS",MCS);
% Get MCS table based on Table 17-4, IEEE 802.11-2020.
...
function [lsigBits,failCheck,preEHTChEst,eqCombDataSym,lsigInfo,eqDataSym,iqImbalance,activeDataSubCarriers] = ehtProcessLSIG(rxLSIG,lltfChanEst,noiseVarEst,chanBW,format)
% Recover L-SIG
...
% estimate
cfgOFDM = wlan.internal.getOFDMConfig(chanBW, 'Long', 'Legacy');
...
% OFDM demodulate
preehtInfo = wlan.internal.ehtOFDMInfo('L-SIG',cfgRx.ChannelBandwidth);
ehtlsigDemod = wlan.internal.ofdmDemodulate(rxLSIG,preehtInfo,0.75);
...
isEHTMU = strcmp(format,'EHT-MU');
if isEHTMU
    % Estimate IQ imbalance
    [iqImbalance, activeSubchan] = wlan.internal.estimateIQImbalance(ehtlsigDemod(preehtInfo.DataIndices,:,:),...
        preEHTChEst(preehtInfo.DataIndices,:,:),preehtInfo);

    % Correct IQ imbalance in channel estimates
    preEHTChEst = wlan.internal.compensateIQImbalanceSIG(preEHTChEst,iqImbalance,preehtInfo,'L-LTF',activeSubchan); 
else
    % HE
    iqImbalance = [0 0];
    activeSubchan = wlan.internal.detectActiveSubchan(ehtlsigDemod(preehtInfo.DataIndices,:,:),preehtInfo.NumSubchannels);
end
% Active (non-punctured) data subcarriers for HE and EHT formats
activeDataSubCarriers = repelem(activeSubchan,52,1);

% Estimate CPE and phase correct symbols
ehtlsigDemod = wlanEHTTrackPilotError(ehtlsigDemod,preEHTChEst(preehtInfo.PilotIndices,:,:),cfgRx,'L-SIG');
...
eqLSIG = ofdmEqualize(ehtlsigDemod,preEHTChEst,noiseVarEst);

if isEHTMU
    % Correct IQ imbalance in equalized symbols
    eqLSIG = wlan.internal.compensateIQImbalanceSIG(eqLSIG,iqImbalance,preehtInfo,'L-SIG');
end

% Extract data symbols
eqDataSym = eqLSIG(preehtInfo.DataIndices,:,:);

% Form demodulated LSIG symbols
ehtlsigDemod = eqLSIG.*preEHTChEst;
...
if ~failCheck && strcmp(chanBW,'CBW20') % Only for 20 MHz for now
    ...
    chanEstMiddle = mean([preEHTChEst(3:54,:,:) remodLSIGChanEst(3:54,:,:)],2);
...
function [bits, failCheck, eqCombDataSym, eqDataSym, activeDataSubCarriers] = lsigRecover(rxLSIG, chanEst, noiseVarEst, chanBW, format)
...
if strcmp(format,'Non-HT')
    % Find active (non-punctured) 20 MHz subchannels for Non-HT
    activeSubchan = wlan.internal.detectActiveSubchan(ofdmOutData,cfgOFDM.NumSubchannels);
else
    % For HT and VHT formats, all 20 MHz subchannels are active (no
    % puncturing)
    activeSubchan = ones(cfgOFDM.NumSubchannels,1);
end
activeDataSubCarriers = repelem(activeSubchan,48,1);
...
[eqCombDataSym, csiCombData] = wlan.internal.equalize(ofdmDataOutOne20MHz, chanEstDataOne20MHz, recParams.EqualizationMethod, noiseVarEst);

% Equalize without combining
eqDataSym = wlan.internal.equalize(ofdmOutData, chanEstData, recParams.EqualizationMethod, noiseVarEst);
