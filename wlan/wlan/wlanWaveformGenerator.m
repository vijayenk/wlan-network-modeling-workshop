...
%   seconds.
%
%   wlanWaveformGenerator (without input or output arguments) launches the
%   WLAN Waveform Generator app for graphical configuration, generation,
%   visualization, and transmission of WLAN waveforms.
%
...
%
%   CFGFORMAT is a format configuration object of type wlanHESUConfig,
%   wlanHEMUConfig, wlanHETBConfig, wlanDMGConfig, wlanS1GConfig, wlanVHTConfig,
%   wlanHTConfig, wlanNonHTConfig, wlanWURConfig, wlanEHTMUConfig, or 
%   wlanEHTTBConfig. The format of the generated waveform is determined by the 
%   type of CFGFORMAT. The properties of CFGFORMAT are used to parameterize
%   the packets generated including the data rate and PSDU length.
%
...
%                              information, see: wlanWaveformGenerator
%                              documentation.
%
...
if nargin == 0
    nargoutchk(0,0);
    % Launch the Wireless Waveform Generator App (with 802.11be default)
    wirelessWaveformGenerator('802.11be')
    return
end
...
validateattributes(cfgFormat,{'wlanVHTConfig','wlanHTConfig','wlanNonHTConfig','wlanS1GConfig','wlanDMGConfig','wlanHESUConfig','wlanHEMUConfig','wlanHETBConfig','wlanWURConfig','wlanEHTMUConfig','wlanEHTTBConfig'},{'scalar'},mfilename,'format configuration object');
...
isEHT = isEHTMU || isa(cfgFormat,'wlanEHTTBConfig');
isOFDM = isHE || isDMGOFDM || isS1G || isVHT || isHT || isWUR || isEHT || ...
...
if nargin==2
    ...
    scramValFn = @(x)validateScramblerInitialization(x,inDSSSMode,isDMG,isNonHT,isWUR,isOFDM,isEHT,cfgFormat);
    winValFn = @(x)validateWindowTransitionTime(x,isOFDM,isHE,isDMG,isS1G,isNonHT,isEHT,cfgFormat);
...
elseif inDSSSMode % DSSS format
    ...
    psps = struct('NumSamplesPerSymbol',0,'CPPerSymbol',0,'ExtensionPerSymbol',0,'NumPacketSamples',0); % For codegen

    preamble = [wlan.internal.dsssPreamble(cfgFormat); wlan.internal.dsssHeader(cfgFormat)];
...
elseif isEHT
    ...
    if isEHTMU
        ...
        preamble = [LSTF; LLTF; LSIG; RLSIG; USIG; STF; LTF];
    end
...
if isWUR
    if useParams.NumPackets > 0
        ...
        if useParams.WindowTransitionTime > 0
            ...
            extensionPerSymbol = 0; % No extension in any symbol
            [preamble,pre,post] = wlan.internal.windowWaveform(preamble,psps(1).NumSamplesPerSymbolPreamble,psps(1).CPPerSymbolPreamble,extensionPerSymbol,wLength); % Window the preamble fields
            ...
            for j = 1:cfgFormat.NumUsers
                subchannel(:,:,j) = wlan.internal.windowWaveform(subchannel(:,:,j),psps(j).NumSamplesPerSymbol,psps(j).CPPerSymbol,extensionPerSymbol,wLength,useParams.NumPackets,numIdleSamples);
            end
        end
    ...
    for i = 1:useParams.NumPackets
        ...
        elseif isHT
            if cfgFormat.PSDULength > 0
                data = wlanHTData(psdu{1},cfgFormat,pktScramInit(i,:),osfNV{:});
        ...
        elseif isNonHT
            if strcmp(cfgFormat.Modulation, 'OFDM')
                ...
                data = wlan.internal.dsssData(psdu{1},cfgFormat);
            end
        elseif isS1G
        ...
        elseif isHE
            if any(psduLength > 0)
                ...
                if Nma>0
                    ...
                    % Added in g3176560 since data may be fixed-size before this if-else but be assigned variable-size in the "if".
                    % This splits "data" into 2 variables so 1 can be fixed-size and the other variable-sized.
                    % TODO: g1378624: remove this workaround.
                    data = data; %#ok<ASGSL>
                end
                dataPacket = data;
...
function validateScramblerInitialization(scramInit,inDSSSMode,isDMG,isNonHT,isWUR,isOFDM,isEHT,cfgFormat)
    ...
    elseif isEHT
        ...
        if isNonHT && isOFDM && ...
                any(strcmp(cfgFormat.ChannelBandwidth,{'CBW20','CBW40','CBW80','CBW160','CBW320'})) ...
...
function validateWindowTransitionTime(tt,isOFDM,isHE,isDMG,isS1G,isNonHT,isEHT,cfgFormat)
    ...
    if isHE || isEHT
        % The 'WindowTransitionTime' value must be less than or equal to
        % twice the minimum cyclic prefix duration within all fields in the
        % waveform. Therefore the cyclic prefix duration of the pre-HE
        % fields limits the maximum 'WindowTransitionTime' to 1.6 us,
        % rather than 6.4 us which is double the longest possible data
