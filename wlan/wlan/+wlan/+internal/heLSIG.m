function [y, bits] = heLSIG(cfgHE,varargin)
%heLSIG Non-HT SIGNAL Field (L-SIG)
%
%   Note: This is an internal undocumented function and its API and/or
%   functionality may change in subsequent releases.
% 
%   Y = heLSIG(CFGHE) generates the Non-HT SIGNAL Field (L-SIG) time-domain
%   signal for the HE transmission format.
%
...
%
%   Y = heLSIG(CFGHE,OSF) generates the L-SIG for the given oversampling
%   factor OSF. When not specified 1 is assumed.
%
%   Example: Generate an L-SIG field for a multiuser HE PPDU format.
%
%     cfgHE = wlanHEMUConfig([193 194]);
%     y = wlan.internal.heLSIG(cfgHE);
%     plot(abs(y));
%
...
% 0 1] representing 6Mb/s in legacy 20MHz CBW, IEEE P802.11ax/D4.1 Section
% 27.3.10.5.
R = [1; 1; 0; 1];
...
cfgOFDM     = wlan.internal.hePreHEOFDMConfig(cfgHE.ChannelBandwidth,'L-SIG');
...
if strcmp(packetFormat(cfgHE),'HE-EXT-SU')
    SF = sqrt(2);
    % Subcarrier based scaling factor only applied to HE-EXT-SU format
...
[lsig,scalingFactor] = wlan.internal.hePreHEFieldMap(dataSymbol,N_LSIG_TONE,cfgHE);
