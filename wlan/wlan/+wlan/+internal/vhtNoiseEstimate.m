function nest = vhtNoiseEstimate(x,chanEstSSPilots,cfgVHT,varargin)
...
% Validate inputs
validateattributes(x,{'double','single'},{'2d','finite'},mfilename,'VHT-Data field input');
validateattributes(chanEstSSPilots,{'double','single'},{'3d','finite'},mfilename,'Single stream pilots channel estimate input');
...
if nargin == 4
    ...
    validateattributes(symOffset,{'numeric'},{'scalar','>=',0,'<=',1},mfilename,'OFDM sampling offset');
else
    symOffset = 0.75;
end

numSTS = sum(cfgVHT.NumSpaceTimeStreams);
cfgOFDM = wlan.internal.getOFDMConfig(cfgVHT.ChannelBandwidth,cfgVHT.GuardInterval,'VHT',numSTS);
...
trackedDemodPilots = wlan.internal.commonPhaseErrorCorrect(ofdmDemodPilots,cpe);

% Estimate noise
pilotError = estRxPilots-trackedDemodPilots;
