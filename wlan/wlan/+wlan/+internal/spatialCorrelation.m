...
NR = width(rxCorr);
NL = NR*NT;
S.AntennaCorrelation = coder.nullcopy(complex(zeros(NL,NL,NP,simConfig.InputDataType)));
for ii = 1:NP
    S.AntennaCorrelation(:,:,ii) = chol(kron(txCorr(:,:,ii),rxCorr(:,:,ii)))';
end
...
function [txCorrelation,rxCorrelation] = antennaCorrelation(modelCfg,simConfig)
    ...
    if strcmpi(simConfig.DelayProfile,'None')
        txCorrelation = eye(numTxAnt,simConfig.InputDataType);
        rxCorrelation = eye(numRxAnt,simConfig.InputDataType);
        return
    end
    ...
    txCorrelation = coder.nullcopy(complex(zeros(numTxAnt,numTxAnt,size(modelCfg.PathPower,2),simConfig.InputDataType)));
    rxCorrelation = coder.nullcopy(complex(zeros(numRxAnt,numRxAnt,size(modelCfg.PathPower,2),simConfig.InputDataType)));
    for ii = 1:size(modelCfg.PathPower,2)
        index = modelCfg.PowerPerAngle(:,ii) > -Inf; % Index of cluster in a path
        assert(any(index),'Expected at least one cluster')
        numClusters = sum(index);
        txCorrelation(:,:,ii) = correlationGeneration( ...
            linspace(0,(numTxAnt-1)*spacingTx,numTxAnt), ...
            numClusters,powerPerAngle(index,ii), ...
            modelCfg.TxAoD(index,ii).',modelCfg.TxAS(index,ii).',180);
        rxCorrelation(:,:,ii) = correlationGeneration( ...
            linspace(0,(numRxAnt-1)*spacingRx,numRxAnt), ...
            numClusters,powerPerAngle(index,ii), ...
            modelCfg.RxAoA(index,ii).',modelCfg.RxAS(index,ii).',180);
    end
end

function R = correlationGeneration(antennaSpacing,numCluster,cluterAmplitude,powerPerAngle,azimuthSpread,angleOfDeparture)
    if numel(antennaSpacing)>1 % numAnt>1
...
function out = channelModelParameters(inp)
    ...
    switch modelType
        case 'None'
            ...
            shadowFading = 0; % Shadow fading standard deviation

            breakPointDistance = 0; % Assume always far-field
        case 'Model-A'
            % Flat fading with 0 ns RMS delay spread(one tap at 0 ns delay)
            % Average power in linear scale
            pathPower = 1;
    ...
    % Power delay profile (PDP) tap spacing as per doc: IEEE 802.11-09/0308r12 (Table-1,
    % page-3). For S1G the relevant TGac channel model changes include only
    % chapters 4 and 5 in IEEE 802.11-09/0308r12 for Multi-user MIMO and
    % Doppler assumptions. See S1G channel doc: IEEE 802.11-11/0968r4.
    switch inp.ChannelBandwidth
        case {'CBW1','CBW2','CBW4','CBW8','CBW16','CBW20','CBW40'}
        ...
        otherwise % 320 MHz
            tapSpacing = 1.25; % In nsec
    end

    % Tap interpolation (TGac)
    bwFactor = 10/tapSpacing; % Bandwidth expansion factor

    if tapSpacing < 10 && ~any(strcmpi(inp.DelayProfile,{'Model-A','None'}))
