...
%
%   [BITS, FAILCRC] = wlanHTSIGRecover(RX, CHANEST, NOISEVAREST, CHANBW)
%   recovers the information bits in the HT-SIG field and performs CRC
%   check.
%
...
%
%   RX is the received time-domain HT-SIG field signal. It is a Ns-by-Nr
%   matrix of real or complex values, where Ns represents the number of
%   time-domain samples in the HT-SIG field and Nr represents the number of
%   receive antennas. Ns can be greater than the HT-SIG field length; in
...
%   and must be 'CBW20' or 'CBW40'.
%
%   [BITS, FAILCRC] = wlanHTSIGRecover(..., NAME, VALUE) specifies
...
chanBW = wlan.internal.validateParam('HTCHANBW', chanBW, mfilename);

% Validate and parse optional inputs
recParams = wlan.internal.parseOptionalInputs(mfilename, varargin{:});

% Get OFDM configuration
ofdmInfo = wlan.internal.vhtOFDMInfo('HT-SIG', chanBW);

% Validate HT-SIG field signal input
validateattributes(rx, {'double','single'}, {'2d','finite','nrows',ofdmInfo.FFTLength*5/2}, mfilename, 'HT-SIG field signal');
numRx = size(rx, 2);
...
coder.internal.errorIf(size(chanEst, 1) ~= ofdmInfo.NumTones, 'wlan:wlanHTSIGRecover:InvalidChanEst1D', ofdmInfo.NumTones);
coder.internal.errorIf(size(chanEst, 2) ~= 1, 'wlan:wlanHTSIGRecover:InvalidChanEst2D');
coder.internal.errorIf(size(chanEst, 3) ~= numRx, 'wlan:wlanHTSIGRecover:InvalidChanEst3D');
...
[demod,varargout{1}] = wlan.internal.htTrackPilotError(demod, chanEst(ofdmInfo.PilotIndices,:,:), chanBW, 'HT-SIG', pilotTrackingParams);

% Merge subchannel channel estimates and demodulated symbols together for
% the repeated subcarriers for data subcarriers
...
[eqDataSym, csiData] = wlan.internal.equalize(ofdmOutDataOne20MHz, chanEstDataOne20MHz, recParams.EqualizationMethod, noiseVarEst);

% Recover bits
[bits, failCRC] = wlan.internal.htSIGBitRecover(eqDataSym, noiseVarEst, csiData);
