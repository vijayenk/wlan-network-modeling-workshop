...
classdef hCaptureIQSamples < handle
%hCaptureIQSamples Captures the IQ samples of a Bluetooth, WLAN, 5G
%NR, or coexistence node
%
%   hCaptureIQSamples(NODE,varargin) captures the IQ samples for
%   Bluetooth, WLAN, 5G NR, or coexistence node.
%
%   hCaptureIQSamples(NODE,Name=Value) captures the IQ samples for
...
%
%   hCaptureIQSamples properties (Configurable as NV Pair):
%
...
%   Copyright 2023 The MathWorks, Inc.
    ...
    methods
        function obj = hCaptureIQSamples(node,varargin)
            % Constructor
            ...
            captureIQFcn = @(varargin) captureSignals(obj,node,networkSimulator,[]);
        ...
        function captureSignals(obj,node,networkSimulator,duration)
            ...
            if currentTime==0
                if ~obj.pIsInitialized
                    ...
                    % periodicity
                    captureIQFcn = @(varargin) captureSignals(obj,node,networkSimulator,duration);
                    ...
                    if round(mod(endTime,duration),9)~=0
                        captureIQFcn = @(varargin) captureSignals(obj,node,networkSimulator,round(mod(endTime,duration),9));
